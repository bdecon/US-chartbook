...
\item {\seriffont \hyperref[sec:oea]{Overall Economic Activity}}

\item {\seriffont \hyperref[sec:ofa]{Overall Financial Activity}}

\item {\seriffont \hyperref[sec:hh]{Households}}

\item {\seriffont \hyperref[sec:bus]{Businesses}}

\item {\seriffont \hyperref[sec:gov]{Government}}

\item {\seriffont \hyperref[sec:ext]{External Sector}}

\item {\seriffont \hyperref[sec:lab]{Labor Markets}}

\item {\seriffont \hyperref[sec:cap]{Capital Markets}}

\item {\seriffont \hyperref[sec:pr]{Prices}}

\item {\seriffont \hyperref[sec:int]{International Comparisons}}

\item {\seriffont \hyperref[sec:ref]{References}}
...
It will also be nice to have a section in that puts some context on numbers generally. For example, put a threshold on GDP growth that marks how much is needed for population growth and depreciation and then calculate how much one extra pp of growth (beyond the previous amount) is worth, per person. Essentially try to get a big number like GDP into a small number. Also, try to emphasize the importance of multiple approaches to GDP compilation. Many people in the US think about GDP only from the expenditure approach, but the income approach is equally important, and arguably more intuitive. \\
...
In many cases, a link to a .csv file contained the data used in the chart \color{magenta}{\faTable}\color{black} \ is available in the bottom right corner of the chart. Each page also has a link to the table of contents \color{blue}{\faList}\color{black} \ in the bottom right corner of the page, to the right of the page number.\\
...
\noindent \hspace*{-2mm} \begin{tikzpicture}
	...
	\draw [dashed] (axis cs:{2002-01-01},\pgfkeysvalueof{/pgfplots/ymin}) -- (axis cs:{2002-01-01},
		\pgfkeysvalueof{/pgfplots/ymax});
	\stdnode{4.1cm}{3.04cm}{\scriptsize historical}
	\stdnode{5.2cm}{3.04cm}{\scriptsize revised}
...
\small Changes to consumer spending (see {\color{black}\textbf{---}}) are largely the result of changes to income (see\cbox{cyan!50!white}) and changes to the rate at which income is saved (see\cbox{magenta!78!purple!80!white}). Changes to other outlays (see\cbox{blue!60!violet}) reflect changes in interest payments, fines and fees, and charitable giving. \\
...
\noindent \hspace*{-3mm} \begin{tikzpicture}
	...
	\sbar{magenta!78!purple!80!white}{date}{SAVING}{data/pcedecomp.csv}
...
\noindent \hspace*{-2mm} \begin{tikzpicture}
  	\begin{axis}[\barplotnogrid axis y line=left, \barylab{3.7cm}{1.5ex}
    	width=5.9cm, bar width=1.7ex, height=9.8cm,
...
\noindent \hspace*{-2mm} \begin{tikzpicture}
	...
	\rbars
	\sbar{orange!70}{date}{FDHBFIN}{data/pubdebt.csv}
	\sbar{green!60!black}{date}{PD}{data/pubdebt.csv}
	\sbar{cyan!50}{date}{IG}{data/pubdebt.csv}
...
\footnotesize{\textit{October 2019, thousands of people, not seasonally adjusted}}\\
...
\footnotesize{\textit{Change from October 2019 to October 2018, thousands of people, not seasonally adjusted}}\\
