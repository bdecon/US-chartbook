...
\small The ``headline'' figure in the monthly jobs report is the change in nonfarm payrolls, which measures how many additional jobs were created or lost in the US in the previous month. The series is particularly important for gauging whether the number of jobs being created is sufficient as the US population grows. Likewise, during a downturn employers typically reduce payrolls, causing families to lose income to the extent that they are not able to make it up through other sources. 
\end{minipage} \hspace{6mm}
\begin{minipage}{0.39\textwidth}
