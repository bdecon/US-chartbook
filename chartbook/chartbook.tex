...
\footnotesize{\textit{voluntary separation share of jobs, percent}}\\
\noindent \hspace*{-2mm} \begin{tikzpicture}
	\begin{axis}[clip=false, stack negative=separate, ymajorgrids=false,
            ...
            width=8.4cm, height=10.4cm, legend style={at={(0.05, 1.05)}},
            \barylab{5.0cm}{1.5ex}]
	\addplot[xbar stacked, bar shift=0pt, fill=none, draw=none] 
		table [y expr=-\coordindex, x index=1] {\quits};
	\addplot[xbar stacked, bar shift=0pt, fill=cyan!60!blue, draw=none] 
		table [y expr=-\coordindex, x index=2] {\quits};		        
	\addplot[only marks, mark=diamond*, mark options={fill=yellow, 
		draw=yellow!50!orange, scale=2}] 
	...
	\end{axis}
\end{tikzpicture}\\
\footnotesize{Source: Bureau of Labor Statistics} \hspace{70mm} \tbllink{quits_ind.csv}\\

\newpage

\noindent \normalsize \textbf{Job Openings Rate by Industry}\\
\footnotesize{\textit{job openings share of jobs, percent}}\\
\noindent \hspace*{-2mm} \begin{tikzpicture}
	\begin{axis}[clip=false, stack negative=separate, ymajorgrids=false,
            yticklabels from table={\quits}{name},
			y axis line style={opacity=0},   
	...
	\addplot[xbar stacked, bar shift=0pt, fill=green!65!blue, draw=none] 
		table [y expr=-\coordindex, x index=2] {\opens};		        
	\addplot[only marks, mark=diamond*, mark options={fill=cyan!50!white, 
		draw=cyan!50!white, scale=2}] 
		table [y expr=-\coordindex, x index=3] {\opens};
	\addplot [nodes near coords={\pgfmathprintnumber{\pgfkeysvalueof{/data point/x}}}, only marks, mark=none] table[y=y, x index=3] {\opens};
	\legend{, 2000-- range, \input{text/jolts_ltdate.txt}}
	\end{axis}
\end{tikzpicture}\\
\footnotesize{Source: Bureau of Labor Statistics} \hspace{70mm} \tbllink{openings_ind.csv}
